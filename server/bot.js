...
// Optional: channel id where /changelog posts and admin-panel changelog
// entries get broadcast as an embed.
...
const isAdmin = (i) =>
  ...
  // anyone with the spider admin role
...
const commands = [
  ...
  // --- admin: role overrides (sticky, independent of Discord) ---
  new SlashCommandBuilder()
    .setName("grantrole")
...
client.once("clientReady", (c) => {
  ...
  // Sweep existing members in each guild so role state is fresh at boot.
...
// Resolve a guild member's Discord roles into Spider's tester/admin booleans
// and push them into the user store so the web/AI pipeline can read them.
async function syncMemberRoles(member) {
  if (!member || member.user?.bot) return;
  await syncUser(member.id); // ensure record exists if they ever logged in
  ...
  await setDiscordRoles(member.id, { tester, admin });
}
